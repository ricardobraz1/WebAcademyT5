...
function editReminder(id: number, updatedReminder: Reminder) {
  const reminders = loadReminders();
...
function renderReminders() {
  ...
  document.querySelectorAll('.edit-btn').forEach(btn => {
    btn.addEventListener('click', (e: any) => {
      ...
      if (reminder) {
        (document.getElementById('reminder-title') as HTMLInputElement).value = reminder.title;
        (document.getElementById('reminder-description') as HTMLInputElement).value = reminder.description;
        (document.getElementById('reminder-date') as HTMLInputElement).value = reminder.dueDate;
        (document.getElementById('reminder-id') as HTMLInputElement).value = id.toString();
      }
    });
  });
...
document.getElementById('reminder-form')?.addEventListener('submit', (e) => {
  ...
  const id = (document.getElementById('reminder-id') as HTMLInputElement).value;
  ...
  // Limpar o formulário
  (document.getElementById('reminder-form') as HTMLFormElement).reset();
});
