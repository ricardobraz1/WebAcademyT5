...
const FormularioLivroPage: FC = (): JSX.Element => {
    ...
    const mostrarErros = (erro: ErroApiDTO) => {
        if (erro?.extras) {
            ...
            adicionarAlerta({
                textoAlerta: erro?.mensagem ?? "Erro desconhecido",
                tipoAlerta: "error",
            });
        }
    };
    ...
    const buscarLivroSelecionado = async () => {
        try {
            const resposta = await ApiService.get<LivroDTO>(`${import.meta.env.VITE_ROTA_LIVROS}/${isbnLivroSelecionado}`);
            if (resposta?.data) {
                ...
                adicionarAlerta({
                    textoAlerta: "Erro ao recuperar livro.",
                    tipoAlerta: "error",
                });
            }
        } catch (e: any) {
            if (e instanceof AxiosError && e.response) {
                mostrarErros(e.response.data as ErroApiDTO);
            } else {
                adicionarAlerta({
                    textoAlerta: "Erro desconhecido ao buscar livro.",
                    tipoAlerta: "error",
                });
            }
            navigate("/");
    ...
    const inserirLivro = async () => {
        ...
        try {
            await ApiService.post<LivroDTO>(import.meta.env.VITE_ROTA_LIVROS, novoLivro);
            ...
            if (e instanceof AxiosError && e.response) {
                ...
                adicionarAlerta({
                    textoAlerta: "Erro ao salvar livro.",
                    tipoAlerta: "error",
                });
            }
        }
    ...
    const atualizarLivro = async () => {
        ...
        try {
            await ApiService.patch<LivroDTO>(`${import.meta.env.VITE_ROTA_LIVROS}/${isbnLivroSelecionado}`, dadosLivro);
            ...
            if (e instanceof AxiosError && e.response) {
                ...
                adicionarAlerta({
                    textoAlerta: "Erro ao atualizar livro.",
                    tipoAlerta: "error",
                });
            }
        }
        ...
        <ContainerColuna>
            ...
            <FormControl sx={{ mt: 1, minWidth: 600 }}>
                <TextField
                    ...
                    multiline
                    rows={4}
                    onChange={(e: ChangeEvent<HTMLInputElement>) => setSinopse(e.target.value)}
                    value={sinopse}
                />
            </FormControl>
            ...
            <FormControl sx={{ mt: 1, minWidth: 600 }}>
                <TextField
                    ...
                    variant="outlined"
                    onChange={(e: ChangeEvent<HTMLInputElement>) => setURLImagem(e.target.value)}
                    value={urlImagem}
                />
            </FormControl>
            ...
            <FormControl sx={{ mt: 1, minWidth: 600 }}>
                <TextField
                    ...
                    variant="outlined"
                    required
                    onChange={(e: ChangeEvent<HTMLInputElement>) => setAutores(e.target.value)}
                    value={autores}
                />
            </FormControl>
